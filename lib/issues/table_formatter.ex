defmodule Issues.TableFormatter do
  ...
  column, and a list of headers for each column return a list containing the
  maximum width of each column (header might be longest too).

  ## Example
      iex> data = [ [ "cat", "wombat", "elk"], ["mongoose", "ant", "gnu"]]
      iex> headers = ["col1", "col2"]
      iex> Issues.TableFormatter.widths_of(data, headers)
      [ 6, 8 ]
  """
  def widths_of(columns, headers) do
    for {column, header} <- Enum.zip(columns, headers) do
      [to_string(header) | column] |> map(&String.length/1) |> max
    end
  end
